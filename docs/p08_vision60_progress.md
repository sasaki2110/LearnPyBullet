...
## 最終更新日
2026年1月15日（最新：PD制御チュートリアルの完成、Franka Pandaを使用した視覚的に分かりやすい往復運動の実装）
...
25. ✅ **歩行動作の実装開始（足踏み後に3m前進する動作を実装）**
26. ✅ **構造調査スクリプトの実装（立ち上がり→安定化後の各モーターの確認）**
...
### 7. PD制御チュートリアルの完成（2026年1月15日 - 最新）
- **背景**: Vision60の制御を理解するため、PD制御の基礎を視覚的に理解できるチュートリアルが必要だった
- **問題**: 初期のチュートリアル（R2D2使用）では、動きが小さすぎて視覚的に確認できなかった
- **解決方法**:
  - **Franka Pandaロボットアーム**を使用：大きなアームが動くので、視覚的に確認しやすい
  - **往復運動**を実装：0度 → 90度 → 0度 を3回繰り返すので、動きがずっと見える
  - **Pゲインを小さく設定**（0.2）：動きを遅くして、PD制御の動作を観察しやすくしている
  - **ログ出力の改善**：角度を度（°）で表示し、目標角度との誤差も表示
- **実装ファイル**:
  - `/root/LearnPyBullet/srcs/pd_tutorial_basic.py` - 基本的なPD制御チュートリアル（往復運動）
  - `/root/LearnPyBullet/docs/PyBullet_PD_Control_Tutorial.md` - PD制御の完全理解チュートリアル
  - `/root/LearnPyBullet/docs/PyBullet_Joint_Control_Guide.md` - ジョイント制御の完全ガイド
- **結果**: 
  - ✅ GUIでアームが往復運動する様子が視覚的に確認できる
  - ✅ ログで角度が0°→90°→0°と変化する様子が数値で確認できる
  - ✅ PD制御の動作（Pゲイン、Dゲインの効果）を理解しやすくなった
  - ✅ チュートリアルドキュメントが完成し、PD制御の基礎を学習できるようになった

## 現在の問題点

### 1. 歩行動作が前進しない（2026年1月15日 - 🔄 進行中）

**問題の概要：**
歩行動作を実装したが、ロボットが前進せず、位置が0.034m～0.067mの範囲で前後に揺れている。目標の3m前進に到達できない。

**実装内容：**
- 足踏み2サイクル完了後に自動的に歩行を開始
- 各フェーズを3段階に分割：
  - 第1段階（0-33%）：対角の足を上げる
  - 第2段階（33-66%）：上げた足を前に出す + 反対の対角を後ろに蹴る
  - 第3段階（66-100%）：前に出した足を伸ばして着地
- フェーズ1と3：基本姿勢に戻す + 反対の対角を後ろに蹴る（前進維持）
- 歩行開始時にオレンジ色に変更
- 歩行中は位置移動によるリセットを無効化

**現在の状態（ログ分析）：**
- 歩行開始位置: (0.111, 0.004, 0.424)
- 歩行距離: 0.034m～0.067mの範囲で前後に揺れている（最大2.2%）
- 位置の推移: X=0.111 → 0.170 → 0.174 → 0.146 → 0.147 → 0.150（前後に揺れている）
- リセット: 発生していない（歩行中のリセット無効化が機能）

**問題点の分析：**
1. **前進力が不足**: 蹴る動作（hipを前向きに回す、kneeを伸ばす）だけでは前進力が不足している可能性
2. **重心移動の問題**: 前に出した足を着地させても、重心が前に移動していない
3. **動作のタイミング**: 各フェーズの動作タイミングが適切でない可能性
4. **実機との違い**: 実機は「ちゃっちゃか進む」が、シミュレーションでは進まない

**試行した修正：**
1. ✅ 後ろに蹴る角度を増加（0.2 → 0.6ラジアン）
2. ✅ kneeを伸ばして後ろに蹴る動作を追加
3. ✅ フェーズ1と3で前進維持動作を追加（基本姿勢に戻す + 反対の対角を後ろに蹴る）
4. ✅ 歩行中のリセットを無効化
5. ✅ 歩行中の姿勢傾き閾値を緩和（30度まで許容）

**次のステップ（次セッション）：**
1. **実機の動作を再確認**: 実機ではどのような動作で前進しているか確認
2. **重心移動の実装**: 前に出した足を着地させた後、重心を前に移動させる動作を追加
3. **前進力の強化**: 蹴る動作の強度やタイミングを調整
4. **動作パターンの見直し**: 実機の動作パターンに合わせて、動作を再設計

**関連ファイル：**
- `/root/LearnPyBullet/agents/p08_vision60/my_agent/utils/walking_control.py` - 歩行制御（修正対象）
- `/root/LearnPyBullet/agents/p08_vision60/my_agent/utils/config.py` - 設定（歩行パラメータ）
- `/root/LearnPyBullet/agents/p08_vision60/my_agent/utils/reset_handler.py` - リセット処理（歩行中の無効化）

**注意**: 歩行动作は一旦中断し、基本的な重心移動（前方転倒チェック）ができるようになってから再開する方針。

### 2. 前方転倒チェックが機能しない（2026年1月15日 - 🔄 進行中）

**問題の概要：**
歩行动作が前進しない問題を受け、基本的な重心移動をテストするため、前方転倒チェック（4本の足を同時に後ろに蹴って前に倒れる動作）を実装した。しかし、ロボットが前に倒れず、ただ沈んでいくだけ。前進距離は0.002m～0.057m程度で、重心が前に移動していない。

**実装内容：**
- 安定化検知後に自動的に前方転倒チェックを開始
- 4本すべての足を同時に後ろに蹴る動作を実装
- 進行度に応じて徐々に強く蹴る（0.0～1.0）
- チェック開始時に赤色（透明度低め、ボディのみ）に変更
- チェック継続時間: 2000ステップ（約4秒）
- hip角度と膝角度のデバッグログを追加

**現在の状態（ログ分析）：**
- チェック開始位置: (0.101, 0.004, 0.416)
- 前進距離: 0.002m → 0.057m（わずか5.7cm）
- 高さ低下: Z=0.416m → 0.235m（約18cm低下）
- 姿勢（pitch）: -3.0° → -2.0°（ほぼ変化なし、前に倒れていない）
- 膝角度の変化:
  - FL: 96.8° → 66.1°（約30度減少）
  - FR: 81.0° → 50.1°（約30度減少）
  - BL: 96.3° → 120.0°（約24度増加）
  - BR: 80.1° → 120.0°（約40度増加）

**問題点の分析：**
1. **重心移動が発生していない**: 4本の足を後ろに蹴るだけでは、重心が前に移動せず、ただ沈んでいくだけ
2. **前に倒れない**: 姿勢（pitch）がほとんど変化せず、前に倒れていない
3. **前進距離が極めて小さい**: わずか5.7cmしか前進していない
4. **左右の脚の動作**: GUIで見ると、左足が動いていないように見える（ログでは膝角度は変化している）

**試行した修正：**
1. ✅ 前脚を前に出す、後脚を後ろに蹴る動作に変更（前脚で重心を前に移動、後脚で反作用で前進）
2. ✅ 4本すべての足を同じように後ろに蹴る動作に統一（左右対称に動作）
3. ✅ デバッグログの追加（hip角度のログ出力）

**次のステップ（次セッション）：**
1. **hip角度の変化を確認**: デバッグログでhip角度が正しく変化しているか確認
2. **左右の脚の動作確認**: GUIで見て左足が動いていないように見える問題を調査
3. **重心移動の実装**: 前に倒れるためには、重心を前に移動させる動作が必要
4. **動作パターンの見直し**: 実機の動作を参考に、より効果的な重心移動パターンを実装

**関連ファイル：**
- `/root/LearnPyBullet/agents/p08_vision60/my_agent/utils/falling_forward_check.py` - 前方転倒チェック（修正対象）
- `/root/LearnPyBullet/agents/p08_vision60/my_agent/utils/config.py` - 設定（前方転倒チェックパラメータ、色設定）
- `/root/LearnPyBullet/agents/p08_vision60/my_agent/main.py` - メインループ（前方転倒チェックの統合）

### 3. 足踏み動作が実機と異なる（2026年1月15日 - ✅ 解決済み）
...
2. **構造調査スクリプトの改善** - 進行中（2026年1月15日）
   - ✅ 立ち上がり→安定化処理を実行してから各モーターの確認を行うように実装
   - ✅ ベース姿勢の変化を表示する機能を追加
   - ⚠️ **課題**: 物理シミュレーションとして正しい動作を確認する方法の検討が必要
     - 他のジョイントの制御を完全に無効化すると重力で倒れる
     - 現在の角度を維持すると物理法則を無視した動きになる
     - より適切な方法を検討する必要がある（例：他のジョイントを弱い力で維持する、または、各ジョイントを動かす時間を短くするなど）

3. ✅ **姿勢の安定性** - 大幅改善（リセット0回、roll=-0.1°, pitch=-0.4°で非常に安定）
4. ✅ **物理パラメータの最適化** - 完了（ジョイント制御の力を30Nに制限、タイムステップを1/480秒に設定）
5. ✅ **リセット処理の改善** - 完了（位置を初期位置に戻す、速度をリセット）
...
### 前方転倒チェックの実装（2026年1月15日 - 最新）
- **実装内容**:
  1. **前方転倒チェックモジュールの作成** (`falling_forward_check.py`):
     - `FallingForwardCheck`クラスを実装
     - 4本すべての足を同時に後ろに蹴る動作を実装
     - 進行度に応じて徐々に強く蹴る（0.0～1.0）
     - チェック継続時間: 2000ステップ（約4秒）
  2. **メインループへの統合** (`main.py`):
     - 安定化検知後に自動的に前方転倒チェックを開始
     - チェック中は足踏み・歩行动作を無効化
     - チェック中は姿勢フィードバック制御を無効化
     - チェック中は位置移動によるリセットを無効化
  3. **色変更機能の追加** (`config.py`):
     - `ROBOT_COLOR_FALLING_FORWARD_CHECK = [0.9, 0.1, 0.1, 0.3]`: 前方転倒チェック中の色（赤系、透明度低め、ボディのみ）
  4. **デバッグログの追加** (`falling_forward_check.py`):
     - hip角度と膝角度のログ出力を追加
     - 100ステップごとにチェックの状態をログ出力
- **問題点**:
  - ⚠️ 前に倒れず、ただ沈んでいくだけ
  - ⚠️ 前進距離が極めて小さい（わずか5.7cm）
  - ⚠️ 姿勢（pitch）がほとんど変化せず、前に倒れていない
  - ⚠️ GUIで見ると、左足が動いていないように見える（ログでは膝角度は変化している）
- **試行した修正**:
  1. ✅ 前脚を前に出す、後脚を後ろに蹴る動作に変更（前脚で重心を前に移動、後脚で反作用で前進）
  2. ✅ 4本すべての足を同じように後ろに蹴る動作に統一（左右対称に動作）
  3. ✅ デバッグログの追加（hip角度のログ出力）
- **次のステップ**:
  - hip角度の変化を確認（デバッグログでhip角度が正しく変化しているか）
  - 左右の脚の動作確認（GUIで見て左足が動いていないように見える問題を調査）
  - 重心移動の実装（前に倒れるためには、重心を前に移動させる動作が必要）
  - 動作パターンの見直し（実機の動作を参考に、より効果的な重心移動パターンを実装）

### 歩行动作の実装（2026年1月15日 - ⏸️ 一時中断）
- **実装内容**:
  1. **状態管理の拡張** (`state.py`):
     - 歩行関連の状態変数を追加（`walking_started`, `walking_start_position`, `walking_phase`など）
     - 足踏みサイクル完了数をカウントする変数を追加
  2. **設定の追加** (`config.py`):
     - 歩行関連の設定を追加（`STEPPING_CYCLES_BEFORE_WALKING=2`, `WALKING_TARGET_DISTANCE=3.0m`など）
     - 歩行动作の角度パラメータを追加（`WALKING_HIP_FORWARD_ANGLE`, `WALKING_KNEE_LAND_ANGLE`など）
     - 総シミュレーションステップ数を35000に増加（歩行時間を確保）
  3. **歩行制御モジュールの作成** (`walking_control.py`):
     - `WalkingController`クラスを実装
     - 各フェーズを3段階に分割（上げる→前に出す→着地）
     - 足踏み2サイクル完了後に自動的に歩行を開始
     - 歩行開始時にオレンジ色に変更
     - 歩行距離を監視し、3m到達で自動終了
  4. **メインループの統合** (`main.py`):
     - 歩行コントローラーを統合
     - 足踏み完了後に自動的に歩行を開始
  5. **リセット処理の改善** (`reset_handler.py`):
     - 歩行中は位置移動によるリセットを無効化
     - 歩行中は姿勢傾きの閾値を30度に緩和
     - 歩行中にリセットが発生しても、位置を初期位置に戻さない
  6. **ログ出力の追加** (`logger.py`):
     - `log_walking_status`メソッドを追加
- **問題点**:
  - ⚠️ 前進せず、位置が0.034m～0.067mの範囲で前後に揺れている
  - ⚠️ 目標の3m前進に到達できない
  - ⚠️ 実機は「ちゃっちゃか進む」が、シミュレーションでは進まない
- **試行した修正**:
  1. ✅ 後ろに蹴る角度を増加（0.2 → 0.6ラジアン）
  2. ✅ kneeを伸ばして後ろに蹴る動作を追加
  3. ✅ フェーズ1と3で前進維持動作を追加（基本姿勢に戻す + 反対の対角を後ろに蹴る）
  4. ✅ 歩行中のリセットを無効化
  5. ✅ 歩行中の姿勢傾き閾値を緩和（30度まで許容）
  6. ✅ 「後ろに引っ張る」動作を削除（後退の原因だった）
- **次のステップ**:
  - 実機の動作を再確認（実機ではどのような動作で前進しているか）
  - 重心移動の実装（前に出した足を着地させた後、重心を前に移動させる）
  - 前進力の強化（蹴る動作の強度やタイミングを調整）
  - 動作パターンの見直し（実機の動作パターンに合わせて再設計）

### 構造調査スクリプトの実装（2026年1月15日 - 最新）
- **実装内容**:
  - `/root/LearnPyBullet/srcs/p08_vision60.py` - 構造調査スクリプトを実装
  - `agents/p08_vision60`の立ち上がり→安定化処理を実行してから、各モーターの確認を行うように実装
  - 立ち上がり→安定化後の状態から、各ジョイントを動かして動作を確認
- **実装した機能**:
  1. **立ち上がり→安定化処理の自動実行**:
     - 環境初期化、初期姿勢設定、姿勢安定化、立ち上がり制御、安定化検知まで自動実行
     - 安定化検知後、100ステップ待機してから各モーターの確認を開始
  2. **各ジョイントの動作確認**:
     - 各脚のジョイント（abduction, hip, knee）を順番に動かして確認
     - 動かすジョイント以外は、現在の角度を維持するように`POSITION_CONTROL`を設定
     - ベース姿勢の変化（roll, pitch, yaw）を表示
  3. **左右対称性の確認**:
     - 前脚と後脚の左右対称性を確認
     - 左右対応するジョイントを同じ角度で動かして比較
     - 移動方向とベース姿勢の変化を表示
- **課題**:
  - ⚠️ **物理シミュレーションとして正しい動作を確認する方法の検討が必要**:
    - 他のジョイントの制御を完全に無効化（`VELOCITY_CONTROL`で`force=0.0`）すると、重力で倒れてしまう
    - 現在の角度を維持（`POSITION_CONTROL`で現在の角度を維持）すると、物理法則を無視した動き（片方の足で立つ）になってしまう
    - より適切な方法を検討する必要がある（例：他のジョイントを弱い力で維持する、または、各ジョイントを動かす時間を短くするなど）
- **調査結果**:
  - ✅ 立ち上がり→安定化後の各ジョイント角度を確認
  - ✅ 左右対称性の確認（前脚のabductionジョイントで、左側が目標角度に到達していない問題を発見）
  - ✅ 各ジョイントを動かした際のベース姿勢の変化を確認
  - ⚠️ 前脚のabductionジョイントで、左側が目標角度に到達していない（可動範囲の制約の可能性）
  - ⚠️ 後脚のabductionジョイントで、左右とも目標角度に到達していない（可動範囲の制約の可能性）

### 足踏み動作の修正と最適化（2026年1月15日）
- **修正1: 股関節の回転方向の修正**
...
- `STEPPING_CYCLES_BEFORE_WALKING = 2`（歩行開始前に必要な足踏みサイクル数）
- `TOTAL_SIMULATION_STEPS = 35000`（総シミュレーションステップ数、歩行時間を確保）
- **足踏み動作の角度調整量（最新）**:
  - 股関節（hip）: 0.3ラジアン（約17度、後ろ向きに回す）
  - 膝（knee）: 0.5ラジアン（約29度、曲げる）
- **歩行动作の角度調整量（最新）**:
  - 足を上げる: hip=0.3ラジアン（後ろ向きに）、knee=0.5ラジアン（曲げる）
  - 前に出す: hip=0.4ラジアン（前向きに）
  - 着地: knee=0.3ラジアン（伸ばす）
  - 後ろに蹴る: hip=0.5ラジアン（前向きに）、knee=0.2ラジアン（伸ばす）
...
## 引き継ぎ事項（2026年1月15日 - 歩行动作実装後）

### 最新の状態（2026年1月15日）
- **リセット回数**: 0回（歩行中もリセット無効化が機能）✅
- **姿勢**: roll=0.2°～0.3°, pitch=-7.6°～6.9°（歩行中は多少傾くが許容範囲内）
- **位置**: 歩行開始位置(0.111, 0.004, 0.424)から、0.034m～0.067mの範囲で前後に揺れている
- **歩行距離**: 最大0.067m（2.2%）、目標3.0mに到達していない ⚠️
- **動作**: 歩行動作は実行されているが、前進していない

### 最新の改善内容（2026年1月15日 - 歩行动作実装）
1. ✅ **歩行动作の実装**: 足踏み2サイクル完了後に自動的に歩行を開始
2. ✅ **歩行中のリセット無効化**: 位置移動によるリセットを無効化、姿勢傾き閾値を30度に緩和
3. ✅ **各フェーズを3段階に分割**: 上げる→前に出す→着地の動作を実装
4. ✅ **色変更機能**: 歩行開始時にオレンジ色に変更
5. ⚠️ **問題**: 前進せず、位置が前後に揺れている

### 現在の課題（2026年1月15日 - 歩行动作実装後）
- **歩行动作が前進しない**: 位置が0.034m～0.067mの範囲で前後に揺れている
  - 実機は「ちゃっちゃか進む」が、シミュレーションでは進まない
  - 蹴る動作だけでは前進力が不足している可能性
  - 重心移動が実装されていない可能性
  - 動作パターンが実機と異なる可能性

### コードの重要な部分（最新 - 2026年1月15日 - 前方転倒チェック実装後）
- **前方転倒チェック**: `agents/p08_vision60/my_agent/utils/falling_forward_check.py`
  - `FallingForwardCheck`クラス: 前方転倒チェックの制御
  - 4本すべての足を同時に後ろに蹴る動作（左右対称）
  - 進行度に応じて徐々に強く蹴る（0.0～1.0）
  - hip角度と膝角度のデバッグログを出力
- **メインループ**: `agents/p08_vision60/my_agent/main.py`
  - 安定化検知後に自動的に前方転倒チェックを開始
  - チェック中は足踏み・歩行动作を無効化
  - チェック中は姿勢フィードバック制御を無効化
  - チェック中は位置移動によるリセットを無効化
- **設定**: `agents/p08_vision60/my_agent/utils/config.py`
  - `ROBOT_COLOR_FALLING_FORWARD_CHECK = [0.9, 0.1, 0.1, 0.3]`: 前方転倒チェック中の色（赤系、透明度低め）
- **前方転倒チェックパラメータ** (`falling_forward_check.py`):
  - `CHECK_DURATION = 2000`: チェック継続時間（ステップ数）
  - `BACK_LEG_KICK_HIP = 0.8`: 後ろに蹴る際のhip角度調整量（ラジアン、前向きに）
  - `BACK_LEG_KICK_KNEE = 0.4`: 後ろに蹴る際のknee角度調整量（ラジアン、伸ばす）

### 歩行动作の状態（2026年1月15日 - ⏸️ 一時中断）
- **歩行动作**: 一旦中断し、基本的な重心移動（前方転倒チェック）ができるようになってから再開する方針
- **問題**: 位置が0.034m～0.067mの範囲で前後に揺れている、目標の3m前進に到達できない
- **関連ファイル**: `agents/p08_vision60/my_agent/utils/walking_control.py`

## 引き継ぎ事項（2026年1月15日 - 以前の内容）
...
## Vision60の構造調査（2026年1月15日）

### 調査の目的
左右のジョイント/モーターの回し方が異なる問題を解決するため、Vision60の構造を完全に把握する。

### 調査スクリプト
- `/root/LearnPyBullet/srcs/p08_vision60.py` - 構造調査スクリプト
  - **実装内容**: `agents/p08_vision60`の立ち上がり→安定化処理を実行してから、各モーターの確認を行う
  - **立ち上がり→安定化処理**: 環境初期化、初期姿勢設定、姿勢安定化、立ち上がり制御、安定化検知まで自動実行
  - **各モーターの確認**: 立ち上がり→安定化後の状態から、各ジョイントを動かして動作を確認

### 調査結果

#### 1. ジョイントと骨格（リンク）の構造

**総ジョイント数**: 16個（可動ジョイント12個 + 固定ジョイント4個）

**各脚の構造**:
- **front_left**: ジョイント [0, 1, 2] (abduction, hip, knee)
- **front_right**: ジョイント [4, 5, 6] (abduction, hip, knee)
- **back_left**: ジョイント [8, 9, 10] (abduction, hip, knee)
- **back_right**: ジョイント [12, 13, 14] (abduction, hip, knee)

**ジョイントの詳細**:
- **abductionジョイント** (0, 4, 8, 12):
  - 可動範囲: [-24.6°, 24.6°] (約±0.43ラジアン)
  - 最大力: 375.0 N
  - 最大速度: 492.7°/s
- **hipジョイント** (1, 5, 9, 13):
  - 可動範囲: [-180.0°, 180.0°] (約±3.14ラジアン)
  - 最大力: 87.5 N
  - 最大速度: 1718.9°/s
- **kneeジョイント** (2, 6, 10, 14):
  - 可動範囲: [0.0°, 180.0°] (約0～3.14ラジアン)
  - 最大力: 87.5 N
  - 最大速度: 1718.9°/s

#### 2. 立ち上がり→安定化後の状態での調査結果

**立ち上がり→安定化後の各ジョイント角度**:
- **front_left**: abduction=0.20°, hip=27.82°, knee=97.42°
- **front_right**: abduction=-0.14°, hip=27.86°, knee=81.32°
- **back_left**: abduction=0.27°, hip=29.44°, knee=96.22°
- **back_right**: abduction=-0.02°, hip=29.44°, knee=80.86°

**立ち上がり→安定化後のベース姿勢**:
- 位置: (0.102, 0.004, 0.416)
- 姿勢: roll=-0.30°, pitch=-2.73°, yaw=0.52°

#### 3. 左右対称性の確認結果（立ち上がり→安定化後）

**前脚の左右対称性**:
- **abductionジョイント** (0 vs 4):
  - 同じ角度（20度）で動かした時:
    - 左側: 角度=2.59°, 移動=(0.000, 0.000, -0.002)
    - 右側: 角度=20.00°, 移動=(0.001, -0.000, -0.017)
    - ベース姿勢変化: roll=-2.15°, pitch=-1.28°
  - **⚠️ 左側が目標角度に到達していない**（可動範囲の制約？）
  - **⚠️ Z方向の移動量が異なる**（左側: -0.002, 右側: -0.017）
- **hipジョイント** (1 vs 5):
  - 同じ角度（30度）で動かした時、移動方向はほぼ同じ
  - ベース姿勢変化: roll=+0.03°, pitch=+0.04°
- **kneeジョイント** (2 vs 6):
  - 同じ角度（30度）で動かした時、移動方向はほぼ同じ
  - ベース姿勢変化: roll=+0.03°, pitch=+0.33°

**後脚の左右対称性**:
- **abductionジョイント** (8 vs 12):
  - 同じ角度（20度）で動かした時:
    - 左側: 角度=0.52°, 移動=(0.001, -0.000, +0.001)
    - 右側: 角度=-0.43°, 移動=(0.001, -0.000, +0.002)
    - ベース姿勢変化: roll=-0.62°, pitch=+0.13°
  - **⚠️ 左右とも目標角度に到達していない**（可動範囲の制約？）
- **hipジョイント** (9 vs 13):
  - 同じ角度（30度）で動かした時、移動方向はほぼ同じ
  - ベース姿勢変化: roll=-0.03°, pitch=-0.06°
- **kneeジョイント** (10 vs 14):
  - 同じ角度（30度）で動かした時、移動方向はほぼ同じ
  - ベース姿勢変化: roll=-0.01°, pitch=+0.24°

**重要な発見**:
- **前脚のabductionジョイントで、左側が目標角度に到達していない**（可動範囲の制約の可能性）
- **後脚のabductionジョイントでも、左右とも目標角度に到達していない**（可動範囲の制約の可能性）
- **各ジョイントを動かした際に、ベース姿勢が変化している**（物理シミュレーションとして正しい動作）

#### 4. 物理シミュレーションとして正しい動作を確認するための試みと課題

**試み**:
- 各ジョイントを動かす際に、他のジョイントの制御を無効化して、物理シミュレーションとして正しい動作（傾き）を確認しようとした

**課題**:
1. **完全に制御を無効化すると重力で倒れる**:
   - `VELOCITY_CONTROL`で`force=0.0`にすると、完全に制御が無効化され、重力で倒れてしまう
   - 姿勢維持制御（PD制御による自動調整）が停止するため、物理シミュレーションとして正しい動作を確認できない

2. **現在の角度を維持すると物理法則を無視した動きになる**:
   - 動かすジョイント以外を現在の角度で`POSITION_CONTROL`で維持すると、姿勢は維持されるが、物理法則を無視した動き（片方の足で立つ）になってしまう
   - 姿勢維持制御は停止するが、現在の姿勢は維持されるため、物理シミュレーションとして正しい動作（傾き）を確認できない

3. **物理シミュレーションとして正しい動作を確認する方法の検討が必要**:
   - 各ジョイントを動かした際の物理的な傾きを確認するには、他のジョイントの制御を完全に無効化する必要があるが、それだと重力で倒れてしまう
   - 現在の角度を維持すると、物理法則を無視した動きになってしまう
   - より適切な方法を検討する必要がある（例：他のジョイントを弱い力で維持する、または、各ジョイントを動かす時間を短くするなど）

### 次のステップ（次セッション）

1. **物理シミュレーションとして正しい動作を確認する方法の検討**
   - **問題**: 各ジョイントを動かす際に、他のジョイントの制御を完全に無効化すると重力で倒れる。現在の角度を維持すると物理法則を無視した動きになる。
   - **検討事項**:
     - 他のジョイントを弱い力（例：10N）で現在の角度を維持する方法
     - 各ジョイントを動かす時間を短くする（例：50ステップ）方法
     - 各ジョイントを動かした直後に、ベース姿勢の変化を記録する方法
     - 他のジョイントを`TORQUE_CONTROL`で現在のトルクを維持する方法

2. **左右対称性の詳細確認**
   - 前脚のabductionジョイントで、左側が目標角度に到達していない原因を調査
   - 後脚のabductionジョイントで、左右とも目標角度に到達していない原因を調査
   - 可動範囲の制約を確認（現在の角度から目標角度までの距離が可動範囲内か）

3. **左右の符号反転の実装**
   - 前脚のabductionジョイントでZ方向の符号が反転していることを確認
   - 制御コードで左右の符号を適切に反転する実装を追加
   - 特に、前方転倒チェックと歩行动作で前脚のabductionジョイントを制御する際に注意

4. **前方転倒チェックの修正**
   - 左右の符号反転を考慮した制御に修正
   - 前脚のabductionジョイントの制御を修正
...
- ✅ 立ち上がり状態から調査できるように改善（完了）
- ✅ より詳細な移動方向の分析を追加（完了、ベース姿勢変化も表示）
- ⚠️ 物理シミュレーションとして正しい動作を確認する方法の検討が必要
- 各ジョイントの回転軸方向の確認を追加
- 足先位置の変化も確認できるように改善

## 引き継ぎ事項（2026年1月15日 - 構造調査スクリプト実装後）

### 最新の状態（2026年1月15日）
- **構造調査スクリプト**: 立ち上がり→安定化処理を実行してから各モーターの確認を行うように実装完了
- **調査結果**: 立ち上がり→安定化後の各ジョイント角度、左右対称性、ベース姿勢の変化を確認
- **課題**: 物理シミュレーションとして正しい動作を確認する方法の検討が必要

### 最新の改善内容（2026年1月15日 - 構造調査スクリプト実装）
1. ✅ **構造調査スクリプトの実装**: 立ち上がり→安定化処理を実行してから各モーターの確認を行うように実装
2. ✅ **ベース姿勢の変化を表示**: 各ジョイントを動かした際のベース姿勢（roll, pitch, yaw）の変化を表示
3. ✅ **左右対称性の確認**: 前脚と後脚の左右対称性を確認（前脚のabductionジョイントで、左側が目標角度に到達していない問題を発見）
4. ⚠️ **課題**: 物理シミュレーションとして正しい動作を確認する方法の検討が必要

### 現在の課題（2026年1月15日 - 構造調査スクリプト実装後）
- **物理シミュレーションとして正しい動作を確認する方法の検討が必要**:
  - 他のジョイントの制御を完全に無効化（`VELOCITY_CONTROL`で`force=0.0`）すると、重力で倒れてしまう
  - 現在の角度を維持（`POSITION_CONTROL`で現在の角度を維持）すると、物理法則を無視した動き（片方の足で立つ）になってしまう
  - より適切な方法を検討する必要がある（例：他のジョイントを弱い力で維持する、または、各ジョイントを動かす時間を短くするなど）

### コードの重要な部分（最新 - 2026年1月15日 - 構造調査スクリプト実装後）
- **構造調査スクリプト**: `/root/LearnPyBullet/srcs/p08_vision60.py`
  - 立ち上がり→安定化処理の自動実行（`agents/p08_vision60`のモジュールを使用）
  - 各ジョイントの動作確認（動かすジョイント以外は現在の角度を維持）
  - 左右対称性の確認（前脚と後脚の左右対称性を確認）
  - ベース姿勢の変化を表示（各ジョイントを動かした際のベース姿勢の変化を表示）
