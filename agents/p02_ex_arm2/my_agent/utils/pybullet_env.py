...
class PyBulletEnvironment:
    ...
    def __init__(self):
        ...
        self.end_effector_index = 11  # Franka Pandaのエンドエフェクタリンクインデックス
        
    def create_environment(self) -> int:
        """
        新しいPyBullet環境を作成（DIRECTモード）
        ...
        try:
            ...
            logger.info(f"✅ [PYBULLET_ENV] PyBullet接続成功 (Client ID: {self.client_id})")
            ...
            # Franka Pandaロボットをロード（土台を地面に固定）
            self.robot_id = p.loadURDF(
                "franka_panda/panda.urdf",
                useFixedBase=True,
                physicsClientId=self.client_id
            )
            logger.debug(f"🤖 [PYBULLET_ENV] Franka Pandaロボットをロードしました (ID: {self.robot_id})")
    ...
    def move_arm_to(self, target_position: Tuple[float, float, float], steps: int = 100) -> bool:
        """
        アームを指定位置に移動（逆運動学を使用）
...
def get_environment() -> PyBulletEnvironment:
    """
    ...
    # 新しい環境を作成
    _global_env = PyBulletEnvironment()
